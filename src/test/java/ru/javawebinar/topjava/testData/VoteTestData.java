...
public class VoteTestData {
    private static final Comparator<Vote> SORT_VOTES_BY_DATETIME_DESC = (o1, o2) -> o2.getVotingDateTime().compareTo(o1.getVotingDateTime());
   ...
   public static final List<Vote> ALL_VOTES_FOR_ITALIAN_WITH_CREATED = collectEntitiesSorted(SORT_VOTES_BY_DATETIME_DESC, ALL_VOTES_FOR_ITALIAN_SORTED, ITALIAN_VOTE_CREATED);
    ...
    private static List<Vote> getAllVotesBetweenDateTimes(List<Vote> votes, LocalDateTime start, LocalDateTime end) {
        return votes.stream().filter(vote -> vote.getVotingDateTime().equals(start) || vote.getVotingDateTime().equals(end)
                || vote.getVotingDateTime().isAfter(start) && vote.getVotingDateTime().isBefore(end)).collect(Collectors.toList());
    }
